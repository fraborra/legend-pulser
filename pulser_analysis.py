...
def main():
    ...
    arg("-p", "--period", nargs=1, action="store", help = "data period")
    ...
    if args["run"]:
        period = int(args["run"][0])
        run    = int(args["period"][0])
...
#    raw_dir = f'/lfs/l1/legend/users/dandrea/test_data/raw/tst/{period}-{run}'
    ...
    # metadata
    version = 'tmp/auto'
#    meta_path = f'/lfs/l1/legend/data/public/prodenv/prod-blind/{version}/inputs/'         # REAL PATH
#    meta_path = '~/temp_files/legend-metadata'                                              # TEMP path
    meta_path = f'/Users/francescoborra/software/legend_200/legend-metadata'
...
#    par_dsp = f'{par_dir}/dsp/cal/p06/r005/l200-p06-r005-cal-20230717T130655Z-par_dsp.json'           # REAL par_dsp
    par_dsp = lmeta.dataprod.overrides.dsp.cal.p03.r000['L200-p03-r000-cal-T%-par_dsp-overwrite']      # TEMP par_dsp
...
#    raw_list = [f'{raw_dir}/{key}_raw.lh5' for key in key_list]
    ...
    key_list = open(f'l200-{period}/{run}/l200-{period}-{run}-tst-key.list', "r")                     # FRANCESCO
...
#                    wcut=2100, plot_dir=plot_dir,lh5_file=lh5_file)
    ...
    for ge_key in ge_keys:                                                                            # FRANCESCO
            pulser_processing(period, run, ge_key, test_dict_file, raw_dir, plot_dir = plot_dir)
            print('Finito un giro', flush=True)
            print('\n\n ===================================== \n\n \n\n', flush = True)
...
#     daq_puls, n_rows = store.read_object(f"./l200-p06-r007-tst-20230726T103316Z-tier_raw.lh5", raw_list)
...
def pulser_processing(period, run, ge_key, test_dict_file, raw_dir, puls_chn = 'ch1027201',
                      nwf = 10, us = 1000/16, plot = False, plot_dir = None, show_plot = False):
    with open(test_dict_file,'r') as file:
        test_dict = json.load(file)
...
#         par_dsp = json.load(file)
    ...
    dsp_dir = run_dir + '/dsp'
    try: os.mkdir(dsp_dir)
    except: FileExistsError: print (f'Directory {dsp_dir} already exists', flush=True)
    else: print(f'Directory {dsp_dir} created', flush=True)
...
#    print(out_voltages)
    ...
    if len(keys)!= len(rough_energies):
        print('There is a problem with the lenght of the attenuation array', flush=True)
        
    pulser_pos = np.zeros(len(keys))
    print('Pulser processing for',ge_key,'String',string,'CC4',cc4_id, flush=True)
...
#        daq_puls = daq_puls
        ...
        # energy spectrum
        energies = dsp_data['cuspEmax']
        ene_mean = np.mean(energies)
        if plot: ene_space = np.linspace(ene_mean-100,ene_mean+100,500)
        ...
        # Save the "normal_dsp" for the analysis
        dsp_out_file = f'./l200-p06/{run}/dsp/big/l200-{period}-{run}-pulser_test-{key}-dsp.lh5'
        store.write_object(dsp_data, name="dsp", lh5_file=dsp_out_file, wo_mode='overwrite')
...
#        cusp_ene = m_cal * cusp_ene + q_cal
        ...
        lh5_file = f'{dsp_dir}/l200-{period}-{run}_pulser-test_acq1.lh5'
        ...
        if lh5_file is not None:
            ...
            try:
                ...
                print('new lh5 file')
#            store.write_object(Array(cusp_ene), name="pulser_energy", lh5_file=raw_file,
#                               wo_mode='overwrite',group=chn)
            store.write_object(Array(energies), name="pulser_energy", lh5_file=lh5_file,
                               wo_mode='overwrite',group=chn)
    ...
    # Save the "small_dsp" for the analysis
    out_lh5_cc4 = f'./l200-p06/{run}/dsp/small/l200-{period}-{run}-small_dsp-{cc4_id}.lh5'

    store.write_object(Array(rough_energies), name="puls ene", lh5_file=out_lh5_cc4,
                       group=f'{chn}', wo_mode='overwrite')
    store.write_object(Array(amplitudes), name="amplitudes", lh5_file=out_lh5_cc4,
                       group=f'{chn}', wo_mode='overwrite')
    store.write_object(Array(attenuations), name="attenuations", lh5_file=out_lh5_cc4,
                       group=f'{chn}', wo_mode='overwrite')
    store.write_object(Array(pulser_pos), name="uncal ene", lh5_file=out_lh5_cc4,
                       group=f'{chn}', wo_mode='overwrite')
    ...
    if plot and show_plot: plt.show()
...
#    lh5_file = f'{raw_dir}/l200-p{06}-{run}-tst-{key}-tier_raw.lh5'
